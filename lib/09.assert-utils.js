...
$dlb_id_au$.unitJS.assert_utils = function() {
  ...
  module.array_compare = function(arr1,arr2) {
    ...
    for(i=0;i<arr1.length;i++) {
      if(arr1[i].constructor == arr2[i].constructor == Array) {
        r = module.array_compare(arr1,arr2);
        if(!r) {
          return false;
        }
      }
      if(!module.compare(arr1[i],arr2[i])) {
        return false;
      }
    }
    return true;
  ...
  module.displayStringForValue = function(aVar) {
    var result = '<' + aVar + '>';
