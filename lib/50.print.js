...
$dlb_id_au$.unitJS.print = function() {
  ...
  // Use this to stop innerHTML causing badness.
  ...
  module.createTests = function(tests) {
    ...
    d3 = node('div',['stats']);
    d3.innerHTML = module.printStats(tests.cumulative);
    d2.appendChild(d3);
  ...
  module.printStats = function(stats) {
    var text = '';
    text += '<span>Pass:</span>'+stats.pass+' ';
    text += '<span>fail:</span>'+stats.fail+' ';
    text += '<span>error:</span>'+stats.error+' ';
    text += '<span>not impl:</span>'+stats.not_implemented+' ';
    text += '<span>assertions:</span>'+stats.assertions+' ';
    return text;
  };
