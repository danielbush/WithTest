...
$dlb_id_au$.unitJS.assertions = function() {
    ...
    // Run the function `test`.  If test is null or falsish value
    // then run will set a NOT IMPLEMENTED status.
    // 
    // If the section for this test has a setup function, it will
    // be executed and the result will be passed to the test.
    //
    // It is assumed that `test` will call assertions
    // defined in this particular instance of the assertions
    // module.

    module.run = function(test,params) {
        var helper = (params && params.helper ? params.helper : null);
        stats = {};
        stats.status = 'PASS';
        stats.assertions = 0;
        stats.assertion_level = 0;
        if(!test) {
            stats.status = 'NOT IMPLEMENTED';
            return stats;
        }
        if(params && params.setup &&
           typeof(params.setup)=='function') params.setup(helper);
        try {
            test.call(params.context,helper);
        } catch(e) {
            stats.unitjs = '';
            stats.message = e.message;  // js error message
            stats.comment = e.comment;  // assertion comment from author
            stats.stack = e.stack;
            if(e.isFailure) {
                stats.status = 'FAIL';
            }
        ...
        if(stats.status == 'FAIL') {
            stats.unitjs+='Assertion #'+stats.assertions+' failed.';
        }
        else if(stats.status == 'ERROR') {
            if(stats.assertions===0) {
                stats.unitjs+='Error occurred BEFORE assertion #1.';
            } else {
                stats.unitjs+='Error occurred ON or AFTER assertion #'+
                    stats.assertions+'.';
            }
        }
        if(params && params.teardown &&
           typeof(params.teardown)=='function') params.teardown();
        return stats;
    }

    function before_assert() {
        if(stats) {
            stats.assertion_level++;
            if(stats.assertion_level==1) {
                stats.assertions++;
            }
        }
    }
