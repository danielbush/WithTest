...
$dlb_id_au$.unitJS.runner = function() {
    ...
    var errors = module.errors = {
        ...
        '005':"Your statements are null or don't exist (this may indicate that you've forgotten to put a comma after a statement or possibly some other syntactical error -- check your browser's error console).",
  ...
  (function() {
    ...
    module.clone = function(thing,obj) {
        ...
        omap(thing,function(label,val){
            if(label=='statements' || label=='tests' ||
               val && (val.section || val.statements || val.tests))
            {
    ...
    // Nested test modules will be converted to nested sections.
    //
    // Notes:
    ...
    //    
    // For every (nested) test module, we set a helper module.  If
    // a helper was not specified for this module, we
    // prevent inheritance by explicitly setting helper to false.
    // Nested sections that weren't packaged as nested test
    // modules *DO* inherit parent test module.

    module.prepare = function(statements,tests,recurse) {

        // Using `recurse` is a little bit of hack.
        // There is probably a more elegant way to write
        // this function (maybe combining `statements`
        // and `tests` into one parameter).

        if(!recurse) {
            statements['helper'] = (tests && tests['helper'] ?
                                    tests['helper'] : false);
        }
        ...
        omap(statements,function(label,val) {
            if(val.statements) {
                if(val.tests) {
                    val.statements['helper'] =
                        (val.tests && val.tests['helper'] ?
                         val.tests['helper'] : false);
                } else val.statements['helper'] = false;

                statements[label] = val.statements;
                module.prepare(val.statements,val.tests,true);
            }
        });
        ...
        omap(tests,function(label,val) {
            ...
            if(label=='setup'||label=='teardown') {
                ...
                module.prepare(statements[label],tests[label],true);
            }
            else {
    ...
    module.process = function(section,results,helper) {
        if(!results) results = {};
        results.section = section.section;

        // Set helper to new helper module if one exists
        // in this section.
        // - If explicitly false, we set helper to false
        //   resulting in assertions.run not using a helper module
        // - if otherwise null or underfined, `helper`
        //   remains what was passed in from a parent
        //   section.

        if(section.helper || section.helper===false) {
        ...
        omap(section,function(label,val) {
            if(!utils.reserved(label)) {
                if(val.fn && val.test) {
                    ...
                    module.process(val,results[label]={},helper);
                }
            }
        });
        return results;
    ...
    (function(){
        ...
        var set_status = function(stats) {
            ...
            if(stats.not_implemented+stats.no_assertions>0) stats.status = 'OTHER';
