...
$dlb_id_au$.unitJS.shoulds = function() {
  ...
  module.it = function(thing) {
    var should;
    //should = new module.Should(thing):
    should = module.should;  // Use one object for everything.
    should.thing = thing;
    return {
      should:should
    };
  ...
  module.Should.prototype = {
    ...
    be:function(thing) {
      ...
      if(this.thing.constructor == thing.constructor == Array) {
        if(!utils.array_compare(theirs,ours)) {
          this._fail(this.thing,thing);
        }
      }
      if(utils.compare(this.thing,thing)) {
      } else {
        this._fail(this.thing,thing);
      }
